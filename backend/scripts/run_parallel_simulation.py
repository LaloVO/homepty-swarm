...
Runs Twitter and Reddit simulations simultaneously using the same config file.

Features:
- Dual-platform (Twitter + Reddit) parallel simulation
- Keeps environments alive after completion, entering command-wait mode
...
# This fixes the issue where third-party OASIS libraries read files without specifying encoding
# ============================================================
import sys
import os
...
if sys.platform == 'win32':
    ...
    # Reconfigure stdout stream to UTF-8 (fixes console Chinese garbled output)
    if hasattr(sys.stdout, 'reconfigure'):
        sys.stdout.reconfigure(encoding='utf-8', errors='replace')
    if hasattr(sys.stderr, 'reconfigure'):
        sys.stderr.reconfigure(encoding='utf-8', errors='replace')

    # Force default encoding (affects open()'s default encoding)
    # Note: this must be set at Python startup; setting it at runtime may not work
    # Therefore we also monkey-patch the built-in open function
    import builtins
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None,
        ...
        """
        # Only set default encoding for text mode (not binary) when encoding is not specified
...
class MaxTokensWarningFilter(logging.Filter):
    """Filter out camel-ai max_tokens warnings (we intentionally omit max_tokens so the model decides)."""

    def filter(self, record):
        # Filter logs containing max_tokens warning
        if "max_tokens" in record.getMessage() and "Invalid or missing" in record.getMessage():
            return False
        return True


# Add filter immediately at module load so it takes effect before camel code runs
...
def disable_oasis_logging():
    ...
    OASIS logs are too noisy (recording every agent observation and action); we use our own action_logger.
    """
...
def init_logging_for_simulation(simulation_dir: str):
    ...
    # Clean old log directory if it exists
...
class ParallelIPCHandler:
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        if success_count > 0:
            ...
            print(f"  Interview completed: agent_id={agent_id}, Successful platforms count={success_count}/{len(platforms_to_interview)}")
            return True
        else:
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    This allows displaying real entity names in actions.jsonl instead of "Agent_0" such aliases
...
def fetch_new_actions_from_db(
    ...
    """
    Fetch new action records from database and supplement full context information
    
    Args:
        db_path: Database file path
        last_rowid: Maximum rowid value read last time (using rowid instead of created_at because different platforms have different created_at formats)
    ...
    try:
        ...
        # This avoids created_at format discrepancies (Twitter uses integers, Reddit uses datetime strings)
        cursor.execute("""
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Simplify action_args, keep only key fields (retain full content, no truncation)
            simplified_args = {}
...
def _enrich_action_context(
    ...
    """
    Enrich action context (post content, user names, etc.)
    ...
    try:
        ...
        # Quote post: supplement original content, author, and quote comment
        ...
        # Create comment: supplement commented post info
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    If boost LLM is configured, parallel simulations can use different API providers per platform to improve concurrency.
...
async def run_twitter_simulation(
    ...
    # Get real agent name mapping from config (use entity_name instead of default Agent_X)
    ...
    for round_num in range(total_rounds):
        ...
        if _shutdown_event and _shutdown_event.is_set():
            if main_logger:
                main_logger.info(f"Received exit signal, stopping at round {round_num + 1} simulation stopped")
            break
        ...
        # Fetch actual executed actions from database and record
    ...
    # Note: environment is not closed, reserved for Interview use
    ...
    log_info(f"Simulation loop completed! Elapsed: {elapsed:.1f}seconds, total actions: {total_actions}")
...
async def main():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Close environment immediately after simulation completion, do not enter command-wait mode'
    )
    
    args = parser.parse_args()
    
    # Create shutdown event at the start of main function to ensure the entire program responds to exit signals
    ...
    if wait_for_commands:
        ...
        # Wait command loop (using global _shutdown_event)
        try:
...
def setup_signal_handlers(loop=None):
    """
    Set up signal handlers to ensure graceful exit upon SIGTERM/SIGINT
    ...
    2. Allow program to clean up resources normally (close database, environment, etc.)
    3. Then exit
    """
    def signal_handler(signum, frame):
        ...
        # Do not directly sys.exit(), let asyncio loop exit normally and clean up
        # Only force exit on repeated received signal
        else:
            print("Forcing exit...")
            sys.exit(1)
