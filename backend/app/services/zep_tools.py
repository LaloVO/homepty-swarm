...
Encapsulates tools such as graph search, node reading, and edge querying for use by the Report Agent.

Core Retrieval Tools (Optimized):
1. InsightForge (Deep Insight Retrieval) - The most powerful hybrid retrieval, automatically generates sub-questions and performs multi-dimensional retrieval.
2. PanoramaSearch (Breadth Search) - Obtains a full view, including expired content.
...
logger = get_logger('homepty_swarm.zep_tools')
...
class PanoramaResult:
    ...
    def to_text(self) -> str:
        ...
        if self.active_facts:
            text_parts.append(f"\n### [Currently Active Facts] (Simulation original text)")
...
class AgentInterview:
    """Interview results for a single Agent"""
    agent_name: str
    ...
    def to_text(self) -> str:
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                if skip:
                    continue
                # Truncate overly long content (truncate by period, not a hard truncation)
...
class ZepToolsService:
    ...
    2. panorama_search - Breadth Search (Obtains a full view, including expired content)
    ...
    - get_all_edges - Get all edges in the graph (including temporal information)
    ...
    def __init__(self, api_key: Optional[str] = None, llm_client: Optional[LLMClient] = None):
        ...
        # LLM client used by InsightForge for generating sub-questions
    ...
    def search_graph(
        ...
        If Zep Cloud's search API is unavailable, it downgrades to local keyword matching.
        ...
        try:
            ...
            if hasattr(search_results, 'nodes') and search_results.nodes:
                for node in search_results.nodes:
                    ...
                    # Node summary is also counted as a fact
    ...
    def _local_search(
        ...
        Obtains all edges/nodes and then performs keyword matching locally.
    ...
    def get_all_nodes(self, graph_id: str) -> List[NodeInfo]:
        """
        Get all nodes in the graph (paginated acquisition)
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        """
        Get all edges in the graph (paginated acquisition, including temporal information)
    ...
    def get_node_edges(self, graph_id: str, node_uuid: str) -> List[EdgeInfo]:
        ...
        Filters all edges in the graph to find those related to the specified node.
        ...
        try:
            # Obtain all edges in the graph and then filter
            all_edges = self.get_all_edges(graph_id)
    ...
    def insight_forge(
        ...
        The most powerful hybrid retrieval function, automatically decomposing questions and performing multi-dimensional retrieval:
        1. Uses LLM to decompose the question into multiple sub-questions.
        2. Performs semantic search for each sub-question.
        3. Extracts relevant entities and obtains their details.
        ...
        # Step 3: Extract relevant entity UUIDs from edges, only obtain information for these entities (do not obtain all nodes)
        entity_uuids = set()
        ...
        # Obtain details for all relevant entities (not limited, full output)
        entity_insights = []
        ...
        for uuid in list(entity_uuids):  # Process all entities, not truncated
            if not uuid:
                continue
            try:
                # Obtain information for each related node individually
                node = self.get_node_detail(uuid)
                if node:
                    node_map[uuid] = node
                    entity_type = next((l for l in node.labels if l not in ["Entity", "Node"]), "Entity")
                    
                    # Obtain all facts related to the entity (not truncated)
                    related_facts = [
    ...
    def _generate_sub_queries(
        ...
        """
        system_prompt = """You are a professional problem analysis expert. Your task is to decompose a complex problem into multiple sub-questions that can be independently observed in the simulated world.

Requirements:
1. Each sub-question should be specific enough that relevant Agent behaviors or events can be found in the simulated world.
...
Return a list of sub-questions in JSON format."""
    ...
    def panorama_search(
        ...
        1. Obtains all relevant nodes.
        2. Obtains all edges (including expired/invalid ones).
        3. Classifies and organizes current valid and historical information.
        
        This tool is suitable for scenarios where a full view of an event or tracking its evolution is needed.
        
        Args:
            graph_id: Graph ID
            query: Search query (used for relevance ranking)
            include_expired: Whether to include expired content (default True)
            limit: Limit for the number of results returned
        ...
        for edge in all_edges:
            ...
            if is_historical:
                # Historical/expired fact, add temporal tag
    ...
    def interview_agents(
        ...
        4. Calls the /api/simulation/interview/batch interface for real interviews (simultaneous interviews on both platforms).
        5. Consolidates all interview results to generate an interview report.
        
        [Important] This function requires the simulation environment to be in a running state (OASIS environment not closed).
        
        [Use Case]
        - Need to understand perspectives on events from different role viewpoints.
        - Need to collect various opinions and points of view.
        - Need to obtain real answers from simulated Agents (not LLM simulations).
        ...
        # Step 2: Use LLM to select Agents to interview (returns list of agent_ids)
        ...
        INTERVIEW_PROMPT_PREFIX = (
            ...
            "6. Answers should have substantive content, with at least 2-3 sentences for each question\n\n"
        )
        optimized_prompt = f"{INTERVIEW_PROMPT_PREFIX}{combined_prompt}"
        
        # Step 4: Call the real interview API (no platform specified, simultaneous interviews on both platforms by default)
        try:
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                # Clean response text: remove tags, numbers, Markdown, and other noise
                ...
                # Strategy 2 (Supplementary): Long text inside properly paired Chinese quotes 「」 or English quotes ""
                if not key_quotes:
            ...
            result.summary = f"Interview failed: {str(e)}. Simulation environment may have closed, please ensure the OASIS environment is running."
            return result
        except Exception as e:
    ...
    def _clean_tool_call_response(response: str) -> str:
        """Clean JSON tool call wrapping in Agent replies, extract actual content"""
    ...
    def _load_agent_profiles(self, simulation_id: str) -> List[Dict[str, Any]]:
        """Load simulated Agent persona files"""
        import os
        import csv
        ...
        # Priority attempt to read Reddit JSON format
        ...
        # Attempt to read Twitter CSV format
    ...
    def _select_agents_for_interview(
        ...
        system_prompt = """You are a professional interview planning expert. Your task is to select the most suitable Agents for an interview from the simulated Agent list based on the interview requirements.
...
Return JSON format:
{
    ...
    "reasoning": "Explanation for the selection reason"
}"""
...
List of available Agents ({len(agent_summaries)} in total):
{json.dumps(agent_summaries, ensure_ascii=False, indent=2)}

Please select at most {max_agents} Agents most suitable for the interview and explain the reasons for your choice."""
        ...
        try:
            ...
            # Obtain full information for selected Agents
            selected_agents = []
            valid_indices = []
...
1. Extract main perspectives from each party.
2. Point out consensus and divergence of opinions.
3. Highlight valuable quotes.
4. Objective and neutral, do not favor any party.
5. Limit to within 1000 words.

Format Constraints (Must be observed):
- Use plain text paragraphs, separated by blank lines.
- Do not use Markdown headers (e.g., #, ##, ###).
- Do not use horizontal rules (e.g., ---, ***).
- {quote_instruction}
- You can use **bold** to mark key terms, but do not use other Markdown syntax."""
