...
Runs simulation in the background and records each Agent's actions, supports real-time status monitoring
"""
...
logger = get_logger('homepty_swarm.simulation_runner')

# Flag to indicate if cleanup function has been registered
...
class SimulationRunState:
    ...
    # Independent rounds and simulated time for each platform (used for parallel display)
    ...
    def to_detail_dict(self) -> Dict[str, Any]:
        """Contains detailed information including recent actions"""
        result = self.to_dict()
...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        if not os.path.exists(config_path):
            raise ValueError(f"Simulation config does not exist, please call /prepare API first")
        ...
        # Truncate if maximum rounds specified
        ...
        try:
            # Build run command, use absolute path
            # New log structure:
    ...
    def _monitor_simulation(cls, simulation_id: str, locale: str = 'zh'):
        """Monitor simulation process, parse action log"""
        set_locale(locale)
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        
        # New log structure: action logs separated by platform
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                if event_type == "simulation_end":
                                    ...
                                    # If only one platform is running, just check that one
                                    # If both platforms are running, both need to be completed
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        ...
        Args:
            ...
            timeout: Timeout to wait for process to exit (seconds)
        """
        if IS_WINDOWS:
            ...
            # Since start_new_session=True was used, PGID equals main process PID
            ...
            try:
                ...
                # If still not exited after timeout, force send SIGKILL
    ...
    def _read_actions_from_file(
        ...
        Args:
            ...
            default_platform: Default platform (used when platform field is missing in action record)
    ...
    def get_all_actions(
        ...
        # If separated platform files don't exist, try reading the old single file format
        if not actions:
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clean up simulation run logs (used for forcing a simulation restart)
        ...
        logger.info(f"Cleanup simulation logs complete: {simulation_id}, deleted files: {cleaned_files}")
    ...
    def cleanup_all_simulations(cls):
        ...
        # Check if there is content to clean up (avoids printing useless logs if there are no processes)
        has_processes = bool(cls._processes)
        has_updaters = bool(cls._graph_memory_enabled)
        
        if not has_processes and not has_updaters:
            return  # No content to clean up, return silently
    ...
    def register_cleanup(cls):
        ...
        # If not in debug mode, this env var doesn't exist, and we still need to register
        ...
        try:
            ...
            logger.warning("Could not register signal handlers (not in main thread), using atexit only")
    ...
    def get_env_status_detail(cls, simulation_id: str) -> Dict[str, Any]:
        ...
        Returns:
            Status details dictionary, including status, twitter_available, reddit_available, timestamp
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specify platform (optional)
                ...
                - None: Interview both platforms simultaneously in dual-platform simulation, returns combined result
        ...
        Raises:
            ValueError: Simulation doesn't exist or environment not running
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (optional, overridden by platform in each interview item)
                - "twitter": Default to interview only Twitter platform
                - "reddit": Default to interview only Reddit platform
                - None: Default to interview both platforms simultaneously for each Agent in dual-platform simulation
            timeout: Timeout in seconds
    ...
    def interview_all_agents(
        ...
        Args:
            simulation_id: Simulation ID
            prompt: Interview question (all Agents use same question)
            platform: Specify platform (optional)
                - "twitter": Interview only Twitter platform
                - "reddit": Interview only Reddit platform
                - None: Interview both platforms simultaneously for each Agent in dual-platform simulation
            timeout: Timeout in seconds
    ...
    def close_simulation_env(
        ...
        try:
            ...
            return {
                "success": True,
                "message": "Environment close command sent (wait response timed out, environment may be closing)"
            }
    ...
    def get_interview_history(
        ...
        Args:
            ...
            platform: Platform type (reddit/twitter/None)
                - "reddit": Get only Reddit platform history
                - "twitter": Get only Twitter platform history
        ...
        if platform in ("reddit", "twitter"):
            ...
            # Query both platforms if platform is not specified
        ...
        # If multiple platforms were queried, limit total results
