...
const chatHistoryCache = ref({}) // Cache for all conversation records: { 'report_agent': [], 'agent_0': [], 'agent_1': [], ... }
...
const selectReportAgentChat = () => {
  ...
  // Restore conversation record for Report Agent
...
const selectAgent = (agent, idx) => {
  ...
  // Restore conversation record for this Agent
...
const renderMarkdown = (content) => {
  ...
  // Clean up cases where <p><br> is followed by a block-level element (caused by extra empty lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Clean up consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Clean up <br> before paragraph start tags follows block-level elements
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix numbering for non-continuous ordered lists: maintain incrementing numbers when single <ol> items are separated by paragraph content
...
const sendMessage = async () => {
  ...
  try {
    ...
    scrollToBottom()
    // Auto-save conversation record to cache
    saveChatHistory()
  }
}
...
const sendToAgent = async (message) => {
  ...
  if (res.success && res.data) {
    ...
    // Convert object dictionary to array, prioritize getting reddit platform replies
    let responseContent = null
    const agentId = selectedAgentIndex.value
    
    if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
      // Prioritize reddit platform replies, then twitter
...
/* Fix ordered list numbering - use CSS counters to let multiple ol's have continuous numbering */
.message-text {
