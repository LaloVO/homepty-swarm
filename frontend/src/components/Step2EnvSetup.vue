...
// State
const phase = ref(0) // 0: Init, 1: Generate Persona, 2: Generate Config, 3: Complete
...
// Log deduplication: record key info of last output
...
// Calculate auto-generated rounds from config (avoid hardcoded defaults)
const autoGeneratedRounds = computed(() => {
  ...
  // Ensure max rounds not less than 40 (recommended) to avoid slider range issues
...
const handleStartSimulation = () => {
  // Build parameters passed to parent component
  const params = {}
  ...
  if (useCustomRounds.value) {
    ...
    // User optional auto-generated rounds, do not pass max_rounds
...
const startPrepareSimulation = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Set expected Agent total immediately (from prepare interface return value)
      ...
      startPolling()
      // Start realtime fetching of Profiles
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Only update if API returns valid values to avoid overwriting existing valid data
...
const loadPreparedData = async () => {
  ...
  // Fetch config (using realtime interface)
  try {
    const res = await getSimulationConfigRealtime(props.simulationId)
    if (res.success && res.data) {
