...
This script reads parameters from the config file to execute simulation, achieving full automation

Features:
- Keeps environment alive after simulation, entering command-wait mode
...
# Load project root .env file (contains LLM_API_KEY and other configurations)
...
class UnicodeFormatter(logging.Formatter):
    """Custom formatter converting Unicode escape sequences to readable characters"""
...
def setup_oasis_logging(log_dir: str):
    """Configure OASIS logs using fixed-name log files"""
...
class TwitterSimulationRunner:
    ...
    def _create_model(self):
        ...
        Use configuration from project root .env file uniformly (highest priority)：
        - LLM_API_KEY: API key
        ...
        if initial_posts:
            ...
            for post in initial_posts:
                ...
                try:
                    ...
                    print(f"  Warning: unable to create initial post for Agent {agent_id}create initial post: {e}")
...
async def main():
    ...
    # Initialize log configuration (using fixed file names, clean old logs)
...
def setup_signal_handlers():
    ...
    Allow program to clean up resources normally (close database, environment, etc.)
    """
