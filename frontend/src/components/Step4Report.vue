...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract statistical data - matches "Related prediction facts: X count" format
    ...
    // Extract core entities - extract all, including summary and relevant facts count
  ...
  try {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // E.g.: - Select Parent_601 (index 0): As representative of parent group...
        if (!headerMatch) {
        ...
        // E.g.: - **Parent_601 (index 0)**: As representative of parent group...
        if (!headerMatch) {
        ...
        if (name) {
          ...
          // Reason continuation (exclude end summary paragraph)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (qMatch) {
        ...
        // Split questions by numbers
        const questions = qText.split(/\n\d+\.\s+/).filter(q => q.trim())
        if (questions.length > 0) {
          // If the first question is preceded by "1.", handle specifically
      ...
      if (answerMatch) {
        ...
        // Platform fallback logic (compatible with old formats: only one platform tag)
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer, copy as default only if it's not placeholder text
          ...
          // No separate platform tags (very old format), used as a whole answer
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Maintain independent platform selection status for each Q&A pair
    ...
    // Check if it's placeholder text for a platform
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // Support 2 numbering formats:
      // 1. "Question X:" or "Question X：" (Chinese format, newer backend format)
      ...
      // If no numbers found or only one found, return as a whole
      if (matches.length <= 1) {
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Split failed: first question returns whole answer, others return empty
      return qIdx === 0 ? answer : ''
    }
    
    // Check if a question has answers for both platforms (filtering placeholders)
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Both platforms have real answers (not placeholders) and different content
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Platform switch buttons (only shown when real dual platform answers exist)
                  ...
                  // Expand/Collapse Button (not shown for placeholders)
...
const renderMarkdown = (content) => {
  ...
  // Remove starting secondary title (## xxx), as section title is already displayed in the outer layer
  ...
  // Clean up <p><br> immediately following block-level elements (caused by extra empty lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Clean up consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Clean up <br> before paragraph start tags immediately following block-level elements
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix non-consecutive ordered list numbering: keep numbering incrementing when single <ol> items are separated by paragraph content
...
const extractFinalContent = (response) => {
  ...
  // Try to find content after Final Answer in Chinese
  ...
  // If content is long and contains markdown format, try to remove thought process
...
/* Console Logs - shared with Step3Simulation.vue */
.console-logs {
  background: #000;
  color: #DDD;
  padding: 16px;
