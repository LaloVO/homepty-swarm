...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate card count for current row to ensure centering
    ...
    // Expand downwards, increase spacing from title
    ...
    const x = offset * 35
    // Adjust starting position, near title but maintaining proper distance
...
// Format simulation_id display (truncate first 6 chars)
...
// Truncate filename (preserves extension)
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update target state (record latest regardless of animation lock)
        ...
        // Use debounce for state switching to prevent rapid flickering
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check if target state still needs execution
          ...
          // Release lock and check for pending changes after animation
          setTimeout(() => {
            isAnimating = false
            
            // Post-animation check for new target state
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Delay slightly before execution to avoid fast switching
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // Adjust rootMargin: shrink bottom of viewport, requires more scroll to trigger expansion
...
onMounted(async () => {
  // Ensure data loads after DOM rendering
  await nextTick()
  await loadHistory()
...
// Reload data when component is activated (if using keep-alive)
onActivated(() => {
  loadHistory()
})
...
/* CSS background pattern for fixed square grid */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* Position from top-left, height changes only extend bottom, doesn't shift grid */
...
.cards-container {
  ...
  /* min-height is dynamically calculated by JS, self-adapting based on card count */
}
...
/* Low saturation color scheme - Morandi spectrum */
