...
logger = get_logger('homepty_swarm.simulation')
...
class SimulationState:
    ...
    def to_simple_dict(self) -> Dict[str, Any]:
        """Simplified status dict (used for API returns)"""
        return {
            "simulation_id": self.simulation_id,
            "project_id": self.project_id,
            "graph_id": self.graph_id,
...
class SimulationManager:
    ...
    3. Use LLM to intelligently generate simulation config params
    4. Prepare all necessary files for preset scripts
    """
    ...
    def prepare_simulation(
        ...
        3. Use LLM to generate simulation config parameters intelligently (time, activity, post frequency, etc.)
        4. Save config files and Profile files
        5. Copy preset scripts to simulation directory
        
        Args:
            simulation_id: Simulation ID
            simulation_requirement: Simulation requirement description (used by LLM for config)
            document_text: Original document content (used by LLM for background)
            ...
            parallel_profile_count: Count of parallel generated personas, default 3
        ...
        try:
            ...
            if filtered.filtered_count == 0:
                ...
                state.error = "No matching entities found, please verify the graph construction"
                self._save_simulation_state(state)
                return state
            ...
            # Pass graph_id to enable Zep retrieval function for richer context
            ...
            # Setup real-time saving file path (prefer Reddit JSON format)
            ...
            # Reddit is already saved in real-time during generation, saving again here ensures completion
            if progress_callback:
                progress_callback(
            ...
            if state.enable_twitter:
                # Twitter uses CSV format! This is a requirement from OASIS
                generator.save_profiles(
                    profiles=profiles,
            ...
            # Note: Running scripts are kept in backend/scripts/ directory, no longer copied to simulation dir
            # When starting simulation, simulation_runner will run script from scripts/ directory
    ...
    def get_run_instructions(self, simulation_id: str) -> Dict[str, str]:
        ...
        return {
            ...
            "instructions": (
                f"1. Activate conda env: conda activate homepty-swarm\n"
                ...
                f"   - Run dual platforms parallelly: python {scripts_dir}/run_parallel_simulation.py --config {config_path}"
            )
        }
