"""
Report API Routes
Provides interfaces for simulation report generation, retrieval, chatting, etc.
"""
...
def chat_with_report_agent():
    ...
    Report Agent can autonomously call retrieval tools to answer questions in the chat
    
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",        // required, simulation ID
            "message": "Please explain...",     // required, user message
...
def get_report_sections(report_id: str):
    """
    Get a list of generated sections (paginated/streamed by section)
...
def get_agent_log(report_id: str):
    ...
    Query parameters:
        from_line: start reading from line number (optional, default 0, for incremental updates)
...
def stream_agent_log(report_id: str):
    """
    Get full Agent logs (all at once)
...
def get_console_log(report_id: str):
    ...
    Real-time retrieval of console outputs (INFO, WARNING, etc.) during report generation.
    This is different from the structured JSON logs returned by the agent-log API,
    as it's a plain-text console-style log.
...
def stream_console_log(report_id: str):
    """
    Get full console logs (all at once)
