...
const handleNextStep = (params = {}) => {
  ...
  // If there are custom rounds, pass through query parameters
  if (params.maxRounds) {
...
/**
 * Check and close the running simulation
 * When user returns from Step 3 to Step 2, default to exiting simulation
 */
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        if (closeRes.success) {
          ...
          // If graceful close fails, try forced stop
          await forceStopSimulation()
        }
      } catch (closeErr) {
        addLog(t('log.closeSimEnvException', { error: closeErr.message }))
        // If graceful close exception occurs, try forced stop
        await forceStopSimulation()
      }
    } else {
