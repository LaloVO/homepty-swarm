  ...
  <div class="process-page">
    ...
    <nav class="navbar">
      ...
      <div class="nav-status">
        <LanguageSwitcher />
        <div class="step-divider" style="width: 1px; height: 14px; background: #333; margin: 0 16px;"></div>
      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        <div class="panel-header">
          ...
          <div class="header-right">
            <template v-if="graphData">
              <span class="stat-item">{{ graphData.node_count || graphData.nodes?.length || 0 }} {{ $t('common.nodes') }}</span>
              <span class="stat-divider">|</span>
              <span class="stat-item">{{ graphData.edge_count || graphData.edges?.length || 0 }} {{ $t('common.relations') }}</span>
              <span class="stat-divider">|</span>
            </template>
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="currentPhase === 1 && !graphData" class="graph-waiting">
            ...
            <p class="waiting-hint">{{ $t('process.dataWillShow') }}</p>
          </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 0, 'completed': currentPhase > 0 }">
            ...
            <div class="phase-detail">
              ...
              <div class="detail-section waiting-state" v-if="!projectData?.ontology && currentPhase === 0 && !ontologyProgress">
                <div class="waiting-hint">{{ $t('process.waitingOntologyDot') }}</div>
              </div>
            </div>
          </div>
...
const statusText = computed(() => {
  ...
  if (currentPhase.value === 1) return t('process.buildingGraph')
  if (currentPhase.value === 0) return t('step1.ontologyGenerating')
...
const goToNextStep = () => {
  ...
  alert(t('common.unknown')) // Placeholder or specific message if added
}
...
// Initialization - Handle new project or load existing
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'No files to upload, please return to home'
    loading.value = false
    return
  }
  ...
  try {
    ...
    ontologyProgress.value = { message: t('progress.analyzingDocs') }
    ...
    if (response.success) {
      ...
      error.value = response.error || t('api.buildFailed', { error: '' })
    }
  } catch (err) {
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      buildProgress.value.message = t('api.graphBuildStarted', { taskId: '' })
...
const fetchGraphData = async () => {
  try {
    // Get project info first to acquire graph_id
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        ...
        buildProgress.value = {
          progress: 100,
          message: t('process.buildingGraph')
        }
