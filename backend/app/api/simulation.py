...
# Adding this prefix prevents the Agent from calling tools, forcing direct text reply
INTERVIEW_PROMPT_PREFIX = "Based on your persona, all past memories and actions, without calling any tools, reply directly with text: "
...
def get_graph_entities(graph_id: str):
    ...
    Only return nodes that match predefined entity types (nodes where Labels are not just Entity)
    ...
    Request (JSON):
        {
            ...
            "graph_id": "mirofish_xxxx",    // optional, if not provided will fetch from project
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    Note: Run scripts (run_*.py) remain in backend/scripts/ dir, no longer copied to simulation dir
    ...
    try:
        ...
        # - preparing: If config_generated=True means prep is complete
        # - running: Running means prep completed earlier
        # - completed: Run complete means prep completed earlier
        # - stopped: Stopped means prep completed earlier
        # - failed: Run failed (but prep was complete)
...
def prepare_simulation():
    ...
    This is a time-consuming operation, the API will return a task_id immediately.
    ...
    5. Save config file and preset scripts
    ...
    Returns:
        {
            ...
            "data": {
                "simulation_id": "sim_xxxx",
                "task_id": "task_xxxx",           // Returned when it's a new task
    ...
    try:
        ...
        if not force_regenerate:
            ...
            if is_prepared:
                ...
                logger.info(f"Simulation {simulation_id} not prepared, starting task")
        ...
        # Allows frontend to immediately get expected Agent count after calling prepare
        try:
            logger.info(f"Synchronously get entity count: graph_id={state.graph_id}")
            reader = ZepEntityReader()
            # Fast fetch (no edges, just count)
            ...
            logger.warning(f"Failed to synchronously fetch entity count (will retry in background): {e}")
...
def get_prepare_status():
    ...
    2. Check if there's completed preparation via simulation_id
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "prepare_info": {...}            // Detailed info if prep is complete
            }
        }
    """
    ...
    try:
        ...
        # If simulation_id is provided, check if preparation is completed
        if simulation_id:
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            if is_prepared:
                return jsonify({
        ...
        # If task_id is missing, return error
        if not task_id:
            if simulation_id:
                # Simulation ID exists but not completed
                return jsonify({
                    "success": True,
                    "data": {
                        "simulation_id": simulation_id,
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    Iterate through reports directory, find reports matching simulation_id,
    ...
    try:
        ...
        # Sort by creation time descending, return newest
...
def get_simulation_history():
    """
    Get history simulation list (with project details)
    
    Used for home page history project display, returns simulation list with rich info like project name, description
            ...
            "data": [
                {
                    ...
                    "simulation_requirement": "If this happens...",
                    "status": "completed",
                    "entities_count": 68,
                    "profiles_count": 68,
    ...
    try:
        ...
        logger.error(f"Failed to get history simulations: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    Get simulation Agent Profiles in real-time (used to monitor progress during generation)
    
    Differences from /profiles API:
    - Reads files directly, bypassing SimulationManager
    - Suitable for real-time monitoring during generation
    - Returns extra metadata (e.g., file modification time, generation status)
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "is_generating": true,  // Whether generation is ongoing
                "file_exists": true,
    ...
    try:
        ...
        # Check if generation is ongoing (via state.json)
        is_generating = False
        total_expected = None
        ...
        logger.error(f"Failed to get realtime Profile: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_simulation_config_realtime(simulation_id: str):
    """
    Get simulation config in real-time (used to monitor progress during generation)
    
    Differences from /config API:
    - Reads files directly, bypassing SimulationManager
    - Suitable for real-time monitoring during generation
    - Returns extra metadata (e.g., file modification time, generation status)
    - Can return partial info even before config generation is complete
...
def get_simulation_config(simulation_id: str):
    ...
    Returns includes:
        ...
        - agent_configs: Activity config for each Agent (activeness, post frequency, stance, etc.)
...
# ============== Profile Generation APIs (Standalone) ==============
...
def start_simulation():
    ...
    About force parameter:
        - When enabled, if simulation is running or completed, it will be stopped and run logs will be cleared
        - Cleared contents include: run_state.json, actions.jsonl, simulation.log, etc.
        - Config files (simulation_config.json) and profile files will not be cleared
        - Suitable for scenarios where the simulation needs to be rerun

    About enable_graph_memory_update:
        - When enabled, all Agent activities (posting, commenting, liking, etc.) in the simulation will be updated to Zep graph in real-time
        - This allows the graph to "remember" the simulation process for later analysis or AI conversations
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        # Get actions for the current round (recent_actions only shows latest round)
...
def get_agent_stats(simulation_id: str):
    """
    Get statistical info for each Agent
...
def interview_agent():
    ...
    Note: This feature requires the simulation environment to be running (enters wait-command mode after completing simulation loop)
...
def interview_agents_batch():
    ...
    Request (JSON):
        {
            ...
            "interviews": [                    // Required, interview list
                ...
                {
                    "agent_id": 1,
                    "prompt": "What do you think about B?"  // If not specified, uses default platform
                }
            ],
    ...
    try:
        ...
        # Optimize prompt for each interview item, add prefix to avoid Agent calling tools
    ...
    Request (JSON):
        {
            ...
            "platform": "reddit",                   // Optional, specify platform (twitter/reddit)
                                                    // If not specified: dual-platform simulation interviews both platforms simultaneously for each Agent
...
def close_simulation_env():
    ...
    Note: This is different from /stop API. /stop will forcefully terminate the process,
    while this API will allow the simulation to gracefully close the environment and exit.
