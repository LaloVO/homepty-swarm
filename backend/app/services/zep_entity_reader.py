...
Read nodes from Zep graph, filter out nodes that match predefined entity types
"""
...
logger = get_logger('homepty_swarm.zep_entity_reader')
...
class ZepEntityReader:
    ...
    2. Filter nodes matching predefined entity types (Labels not just Entity)
    3. Get related edge and node info for each entity
    """
    ...
    def _call_with_retry(
        ...
        Args:
            func: Function to execute (no args lambda or callable)
            operation_name: Operation name for logging
            max_retries: Max retry count (default 3 times, meaning at most 3 attempts)
    ...
    def get_node_edges(self, node_uuid: str) -> List[Dict[str, Any]]:
        """
        Get all related edges of a specific node (with retry mechanism)
        ...
        """
        try:
            # Call Zep API using retry mechanism
    ...
    def filter_defined_entities(
        ...
        Filtering logic:
        - If node's Labels only contain "Entity", this entity does not match predefined types, skip
        - If node's Labels contain tags other than "Entity" and "Node", it matches predefined types, keep
    ...
    def get_entity_with_context(
        ...
        """
        try:
            # Get node using retry mechanism
