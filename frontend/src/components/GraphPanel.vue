  ...
  <div class="graph-panel">
    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <!-- Hint after simulation finishes -->
...
const renderGraph = () => {
  ...
  // Process edge data, count edges between same node pairs and calculate index
  const edgePairCount = {}
  const selfLoopEdges = {} // Self-loop edges grouped by node
  const tempEdges = edgesData
    .filter(e => nodeIds.has(e.source_node_uuid) && nodeIds.has(e.target_node_uuid))
  
  // Count edge numbers between each pair of nodes, collect self-loops
  tempEdges.forEach(e => {
  ...
  // Record current edge index for each node pair
  const edgePairIndex = {}
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // Self-loop - add only one merged self-loop per node
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // Store detailed info for all self-loops
        }
      })
      return
    }
    ...
    let curvature = 0
    if (totalCount > 1) {
      // Curvature range increases with edge count for clear distinction
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // If edge direction is opposite to standardized, flip curvature
      // This ensures all edges spread in the same reference frame
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add attraction to center for disconnected node groups
  ...
  const getLinkPath = (d) => {
    ...
    // Offset perpendicular to line direction, proportional to distance for visibility
    // Ratio increases with edge count
  ...
  // Link labels background (White background for better clarity)
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Record position only, don't restart simulation (distinguishes click from drag)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // Detect if actually dragging (moved beyond threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // Only restart simulation upon detecting real drag
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Gradually stop simulation only if actually dragged
        if (d._isDragging) {
...
/* Styles for hint after simulation ends */
